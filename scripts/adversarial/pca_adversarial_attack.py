...
def main(model, dataset, margin, seed, projection_dim=256):
    ...
    logger.info(f"Loading PCA projection layer from: {pca_projection_save_path}")
    
    projection_layer = nn.Linear(in_features=MODEL.out_features, out_features=projection_dim)
    projection_layer.load_state_dict(torch.load(pca_projection_save_path, map_location=device))
    ...
    gallery_path = f'features/pca/{path_creator.create_path(MODEL.name, DATASET.name, output_dim=out_dim, attack_name=None, iter_count=iter_count, margin=None, seed=SEED)}/features.pt'
    ...
    adv_features_save_path = f'features/pca/{path_creator.create_path(MODEL.name, DATASET.name, output_dim=out_dim, attack_name=type(attack).__name__, iter_count=iter_count, margin=attack.margin, seed=SEED)}'
...
if __name__ == '__main__':
    for model in [CLIPVitB()]:
        for dataset in [SOPDataset()]:
            for margin in [0.1, 0.2, 0.5]:
            # for margin in [1/255, 2/255, 4/255, 8/255]:
                for seed in [31]:
                    main(model, dataset, margin, seed, projection_dim=256)